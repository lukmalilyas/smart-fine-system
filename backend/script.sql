...
CREATE TABLE Domain.VehicleOwners (
...
CREATE TABLE Domain.Fines (
    ID INT PRIMARY KEY AUTO_INCREMENT,                              -- Unique ID (auto-incremented)
    ...
    CONSTRAINT FK_VehicleOwners FOREIGN KEY (LicensePlate) 
        REFERENCES Domain.VehicleOwners(LicensePlate) ON DELETE CASCADE -- Ensuring referential integrity
);
